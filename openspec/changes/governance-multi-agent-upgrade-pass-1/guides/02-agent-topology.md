...
| Agent | 主要职责 | 关键输出 |
|-------|----------|----------|
| `orchestrator` | 任务分类、激活 agent、管理 handoff、整合结果 | 执行计划、激活决策、最终总结 |
...
|----------|-----------|-------------|
| `intake` / `request` | orchestrator, research-author-agent | governance-review-agent（仅治理改造） |
| `source` | orchestrator, source-evidence-agent | 无 |
| `plan` | orchestrator, research-author-agent, source-evidence-agent | 无 |
| `draft` | orchestrator, research-author-agent | diagram-agent |
| `review` | orchestrator, review-critic-agent | diagram-agent（如存在图） |
| `merge/apply` | orchestrator, publish-agent | governance-review-agent（治理类变更不 apply 到 knowledge） |

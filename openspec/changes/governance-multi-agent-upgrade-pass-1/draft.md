...
本轮改造把仓库执行层从”单 agent 顺序执行 + 阶段命令分散约定”升级为”以命令层为入口、以 `Harness` 为真源、以 agent contract 为边界”的第一版 multi-agent 模式。
...
- 【S1】命令层读取 `AGENTS.md`、workflow index、agent index，判断这是普通 research、update，还是 governance 改造。
- 【S2】如为普通 research，命令层激活 @research-author-agent 负责 `request / plan / draft` 主链，同时按需要并行激活 @source-evidence-agent 与 @diagram-agent。
...
- 以 `commands + agent contracts` 驱动 Claude 命令层
- 以 `4 + 2` 角色模型覆盖主要研究路径
- 对关键旧入口做第一轮对齐，减少错误路由
...
2. 第一版以 `4 + 2` 角色模型最合适：既保留独立 reviewer，又避免 roster 膨胀。

...
**primitive 类型必须覆盖的问题层次**：
1. 定义层：是什么/管什么/不管什么（例：「这个协议解决什么问题？不解决什么？」）
2. 范围/边界层：能力范围与边界（例：「协议的适用范围在哪里？与相邻协议的边界？」）
3. 机制层：核心机制如何工作（例：「核心组件有哪些？关键流程如何流转？」）
4. 关系层：与相邻对象的关系（例：「与 X 协议是替代/互补/演进关系？」）

**注意**：primitive 类型必须包含定义层和范围/边界层的问题，不得直接跳入机制细节。
...
对于 synthesis/decision 类型，建议在此初步列出依赖的 primitive 对象：
- 依赖的 primitive 列表
- 每个 primitive 的初步深度判断
-->
...
示例：
- primitive：不覆盖具体钱包产品的实现
- synthesis：不深入每个对象的完整机制（由 primitive 处理）

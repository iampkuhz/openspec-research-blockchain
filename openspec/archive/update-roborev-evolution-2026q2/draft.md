...
> **说明**：以下 PlantUML 图代码已嵌入正文，需通过 `feipi-plantuml-generate-architecture-diagram` 和 `feipi-plantuml-generate-sequence-diagram` skills 校验语法正确性。
...
2026-01-05 ─────────── 02-23 ──────── 02-24 ────────── 03-17 ─────────────── 03-18 ───── 至今
  ...
  │  Gemini/Copilot/OpenCode/Kilo      │                                        │  auto_close_passing_reviews
...
**核心技术思考**："完整架构先行"。RoboRev 在 repo 创建后几天内就搭建了 CLI + daemon + TUI + SQLite + 双 agent 的完整框架，后续所有功能都在此框架上增量填充，而非从零渐进构建。这反映了项目定位明确、设计者对整体架构有清晰蓝图的特征。

### 大爆炸式初始架构（2026-01-05 ~ 01-09）

- **repo 创建**：2026-01-05，但首次公开 commits 集中在 2026-01-09，表明初始架构可能在私有开发中完成，开源时一次性发布 【L2, GH-REPO-META + GH-COMMITS-EARLY】
...
- **技术栈确认**：Go + `modernc.org/sqlite`（纯 Go SQLite，无 CGO 依赖）+ Bubble Tea 【L2, GH-GOMOD】
...
- **beads 集成**：作为 hook type 集成，在 review 失败时通过 `HookConfig{Type: "beads"}` 自动创建可追踪 issue 【L2, GH-CONFIG + GH-README】
...
- **协议来源**：ACP 是 **Coder 公司**（code-server 的开发者）提供的外部 SDK/协议，通过 `github.com/coder/acp-go-sdk v0.6.3` 集成到 RoboRev。这不是 RoboRev 的内部发明 【L2, GH-GOMOD + CODER-ACP-SDK】
- **协议效果**：ACP 引入后 3 天内（v0.40.0 ~ v0.42.0）即接入了 Kiro、Cursor、Pi 三种 agent，相比此前每个 agent 需数天的适配周期，接入速度显著提升
- **双路径并存**：`internal/agent/` 目录中，CLI agent 文件（`codex.go`, `claude.go`, `copilot.go`, `gemini.go`, `opencode.go`, `kilo.go`, `kiro.go`, `cursor.go`, `pi.go`, `droid.go`）与 ACP agent 文件（`acp_agent.go`）**同时存在**。CLI agent 未被迁移到 ACP，两条路径并行运行 【L2, GH-INTERNAL-AGENT】
- **ACP Agent 配置**：通过 `ACPAgentConfig` 配置（command、args、model、read_only_mode、auto_approve_mode），默认命令为 `acp-agent` 【L2, GH-INTERNAL-AGENT】
- **Agent 注册与 Fallback**：`Register(a Agent)` 注册，`Get(name string)` 获取，`GetAvailable(preferred, backups...)` 实现 fallback 链 【L2, GH-INTERNAL-AGENT】

### Fix/Refine 闭环

- **`roborev fix`**：针对单个未通过的 review，调用配置的 fix agent 生成修复补丁
- **`roborev refine`**：自动循环 — fix → 重审 → 如果仍不通过则继续 fix，直到通过或达到上限
- **终止条件**：`refine.go` 源码确认 `--max-iterations` 标志，默认值为 **10** 次 【L2, GH-CMD-REFINE】
...
- **Webhook hooks（v0.44.0）**：外部系统可通过 webhook 触发审查，`HookConfig{Type: "webhook", URL: "..."}` 【L2, GH-CONFIG】
...
- **双存储后端**：`internal/storage/` 目录中同时存在 `db.go`（SQLite）和 `postgres.go`（PostgreSQL），通过 `github.com/jackc/pgx/v5` 实现 【L2, GH-GOMOD + GH-INTERNAL-STORAGE】
...
- **实现机制**：通过 `git worktree add --detach` 创建临时 worktree，设置 `core.hooksPath=os.DevNull` 禁用 hooks，初始化 submodules，可选拉取 LFS 文件。完成清理时调用 `git worktree remove --force` + `os.RemoveAll` 【L2, GH-INTERNAL-WORKTREE】
- **非容器化**：沙箱是 git worktree 级别的隔离，不是 Docker/OCI 容器化。这是轻量级方案，对于解决 git lock 竞争问题已经足够
- **refine 也使用 worktree**：`refine` 命令同样使用 worktree 来隔离 fix/commit 操作与用户的 working tree 【L2, GH-INTERNAL-WORKTREE】
...
- **service/socket units**：`packaging/systemd/` 目录提供 `roborev.service` 和 `roborev.socket` 【L2, GH-PACKAGING-SYSTEMD】
...
| 早期私有开发 | 未解决 | repo 创建日 2026-01-05 到首次公开 commits 2026-01-09 之间是否有私有开发无法确认 |
...
1. **RoboRev 是 commit 级 AI code review 工具**：通过 post-commit hook、CI pipeline、webhook 三种触发方式，对 AI agent 产出的 commit 进行自动化审查。 【L2, README + release notes】

2. **初始架构采用"大爆炸式"策略**：repo 创建（2026-01-05）到首次公开（2026-01-09）期间即完成了 CLI + daemon + TUI + SQLite + 双 agent 的完整架构搭建。后续所有功能都在此框架上增量填充。 【L2, repo metadata + commit history】

3. **ACP 是 Coder 的外部 SDK，非 RoboRev 内部协议**：通过 `github.com/coder/acp-go-sdk v0.6.3` 集成，这是本次回源最重要的修正。ACP 引入后新 agent 接入速度显著提升，但已有的 CLI agent（Codex、Claude Code、Gemini 等）并未迁移，两条路径并存。 【L2, go.mod + agent/ 源码】
...
5. **fix/refine 闭环将 RoboRev 从"发现问题"升级为"解决问题"**：`fix`（单次修复）+ `refine`（自动循环，`--max-iterations` 默认 10）+ `auto_close`（自动关闭 PR）形成完整的质量保障链。 【L2, refine.go 源码 + release notes】

6. **沙箱是 git worktree 隔离，非容器化**：通过 `git worktree add --detach` + `core.hooksPath=os.DevNull` 实现轻量级隔离，足以解决 `.git/index.lock` 竞争问题。 【L2, worktree.go 源码】

7. **beads 集成确实存在**：作为 hook type（`HookConfig{Type: "beads"}`），review 失败时自动创建可追踪 issue。这是已存在的集成，不是深度架构组件。 【L2, README + config.go】

8. **存储已从 SQLite-only 扩展到双后端**：`internal/storage/` 同时支持 SQLite 和 PostgreSQL（pgx/v5），满足不同规模部署需求。 【L2, storage/ 源码 + go.mod】
...
| 早期私有开发 | **未解决** | 2026-01-05（创建日）到 01-09（首次公开）之间的开发活动无法确认 |
...
| [[本地 artifact]基线 artifact](knowledge/analysis/primitives/ai-code-review/roborev-evolution/artifact.md) | L4 | [已读取] 参考基线，多项主张已被 L2 证据修正 |

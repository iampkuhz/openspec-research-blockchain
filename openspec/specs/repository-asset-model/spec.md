...
- 长期正式正文默认使用 `reference.md`，而不是过程中的 `draft.md`。
- `request.md`、`plan.md` 这类过程 artifacts 必须放在 `openspec/changes/<change-name>/` 下。
- `draft.md` 属于过程层集中 review 文件，不得直接作为长期正式稿放入 `knowledge/`。
- `evidence-matrix.md` 这类过程性证据组织文件默认保留在 `openspec/changes/<change-name>/` 下。

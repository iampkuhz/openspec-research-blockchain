...
- `knowledge/analysis/` 默认只保留 `artifact.md`
- glossary 层默认折叠进 `artifact.md` 的“关键术语”区
- `knowledge/decisions/` 保留 `artifact.md`、`criteria.md`、`verdict.md`

### artifact.md 结构约束

**必须包含的章节**（按此顺序，根据对象类型适配）：

1. **目录**（自动生成的导航目录）
2. **关键术语**（表格：术语、定义、作用）
3. **分析正文**（根据对象类型不同）：
   - primitive：组件架构 → 核心流程
   - synthesis：演进框架 → 各对象定位 → 演进关系分析
   - domain：问题簇划分 → 与相邻 domain 关系
   - decision：场景定义 → 比较维度 → 选项分析
4. **设计取舍**（为什么这样设计，而不是那样设计）
5. **能力边界**（能解决什么、不能解决什么、失败条件）
6. **相关协议关系**（与相邻协议/对象的关系定位）
7. **可确认结论**（基于证据的有限结论，不写证据等级标注）
8. **Evidence Gap**（已知的证据缺口）
9. **参考资料**（简化格式，见下方）
...
- **必须保留所有核心图表**（演进时间线、组件架构、流程图、对比表格等）
- **移除 PlantUML 代码块标记**，但保留图表的文字描述和说明
- **图表优先、文字补充**：artifact.md 仍需遵循图表优先原则
- **分层图表保留**：主框架图和关键子阶段图都应保留

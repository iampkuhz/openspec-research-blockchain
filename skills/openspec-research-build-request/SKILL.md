...
- `harness/workflows/request-phase.md` —— request 阶段规范（入口）
- 相关上位规范（见 `request-phase.md` 中"与上位规范的关系"）

本 skill 不复制上位规范正文，仅负责 Qoder 的触发入口、使用时机与输入输出。

若 `request-phase.md` 与其引用的上位规范存在差异，以相关上位规范为准。
...
如用户未提供足够上下文，按 `harness/workflows/request-phase.md` 补齐 request 所需关键信息：

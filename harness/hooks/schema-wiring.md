...
flowchart TD
    ...
    B[schema.yaml]
    C[base.schema.yaml]
    D[profiles/*.schema.yaml]
    E[operations/*.schema.yaml]
    F[templates/change.yaml]
    ...
    B -->|引入公共契约| C
    B -->|按 task_type 加载 profile| D
    B -->|按 change_operation 加载 operation| E
    B -->|提供 change 模板| F
...
2. schema.yaml
   → imports.base 指向 ./base.schema.yaml（公共契约）
   → 根据 task_type 选择 profiles/*.schema.yaml
   → 根据 change_operation 选择 operations/*.schema.yaml
   → 提供 templates/* 作为模板源
...
3. **schema 是统一的**：`schema.yaml` 是入口，不是四套独立 schema。base + profiles + operations 共同构成一个 schema package。
